    ...
    def __init__(self, text):
        ...
        for word in split_text :
            ...
            if word_minus_punc not in self._word_data:
                  word_data = {'count':0, 'word':word_minus_punc, \
                    'neighbors':[]}
            ...
            if index > 0:
                ...
                '''if the previous word ended a sentence then don't
                include it as a neighbor'''
                ...
                formated_previous = ''
            '''if the current word ends a sentence then don't
            give it a forward neighbor'''
            if index < len(split_text)-1 and word[-1] not in ".!?":
                next_word = split_text[index+1]
                formated_next = self._format_text(next_word)
            else:
                formated_next = ''
            word_data['neighbors'].append({'previous_neighbor':\
              formated_previous, 'next_neighbor':formated_next})
            word_data['count'] += 1
            index += 1
